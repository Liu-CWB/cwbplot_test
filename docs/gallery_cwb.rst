Gallery
=====

Basemap
-----
The projection setting of this graph is done by basemap, but strongely change to use cartopy. 

.. hlist::
    :columns: 3

    - .. figure:: image/gallery/12acc_rainfall_basemap.png
        :width: 600
        :target: https://cwbplot-test.readthedocs.io/en/latest/example/12accrain_basemap.html
        
        Rainfall
    
    - .. figure:: image/gallery/surftemp_basemap.png
        :width: 600 
        :target: https://cwbplot-test.readthedocs.io/en/latest/example/surftemp_basemap.html
        
        Surface Temperature and Sea Level Pressure
        
    - .. figure:: image/gallery/two_colorbar.png
        :width: 600 
        :target: https://cwbplot-test.readthedocs.io/en/latest/example/surftemp_basemap.html
        
        Two colobar in a graph
        

.. hlist::
    :columns: 3

    - .. figure:: image/gallery/rh_streamline_basemap.png
        :width: 300
        :target: https://cwbplot-test.readthedocs.io/en/latest/example/rh_streamline_basemap.html
        
        Relative humidity and streamline
    
    - .. figure:: image/gallery/CHANSHU.png
        :width: 600 
        :target: https://cwbplot-test.readthedocs.io/en/latest/example/tytrack_basemap.html
        
        Typhoon Track
        
    - .. figure:: image/gallery/Vertical_cross_section.png
        :width: 600
        :target: https://cwbplot-test.readthedocs.io/en/latest/gallery_cwb.html
        
        Vertival cross section for grib2 data format


.. toctree::
    :maxdepth: 1
    :hidden:

    example/12accrain_basemap
    example/surftemp_basemap
